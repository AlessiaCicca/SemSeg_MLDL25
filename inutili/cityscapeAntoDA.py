...
class CityscapesNoLabel(Dataset):
    def _init_(self, annotations_file, transform=None):
        self.img_labels = pd.read_csv(annotations_file)
        self.transform = transform
...
# ------------------------------
# Parametri dataset e trasformazioni
# ------------------------------
...
def ensure_cityscapes_dataset():
    images_dir = find_folder(extract_dir, 'images')
    gtfine_dir = find_folder(extract_dir, 'gtFine')

    if not os.path.exists(extract_dir) or not images_dir or not gtfine_dir:
...
def create_csv_no_labels(images_dir, output_csv):
    ...
    print(f"Creato CSV con {len(image_files)} immagini: {output_csv}")c
